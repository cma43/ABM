...
class Environment(object):
    ...
    def tick(self):
        ...
        # Keeps track of the locations of crimes committed this step
        self.crimes_this_turn = list()

        # commit a crime
        for g in self.coalitions:
            # If crime is committed, the crime location is returned
            ...
            if crime:
                # Add to placeholder for use later
                self.crimes_this_turn.append([crime[0], crime[1]])

        # Add recent crimes to list of crime places that still need to be dispatched to
        self.crime_place += self.crimes_this_turn

        # ------ police move
        random.shuffle(self.police)

        if self.config['police_dispatch'] == 'random':
            # A random police officer is dispatched to crime scene
            moved_police = list()
            for i in range(len(self.police)):
                # move to the crime place immediately
                if len(self.crime_place) > 0:
                    self.police[i].move(self.config['grid_width'], self.config['grid_height'], self.crime_place[0][0],
                                        self.crime_place[0][1])
        ...
        # FIXME Can we make this more efficient?
            ...
            # The closest police officer who hasn't moved yet is dispatched to the crime scene
            moved_police = list()
            ...
            for crime_scene in self.crime_place:
                ...
                if len(remaining_police) == 0:
                    break
                closest_police, closest_distance = remaining_police[0], self.police[0].dist(crime_scene[0],
                                                                                            crime_scene[1])
    ...
    def populate(self):
        ...
        # Populate Police
        for police_id in range(self.config['num_criminals'] + self.config['num_civilians'],
                               self.config['num_criminals'] + self.config['num_civilians'] + self.config['num_police']):
            self.police.append(
